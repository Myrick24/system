...
class _UnifiedMainDashboardState extends State<UnifiedMainDashboard> {
  ...
  // Cache seller status to avoid repeated queries
  static final Map<String, bool> _sellerStatusCache = {};
  ...
  Future<void> _checkSellerStatus() async {
    ...
    if (_lastCacheUpdate != null &&
        _sellerStatusCache.containsKey(currentUser.email) &&
        now.difference(_lastCacheUpdate!).inMinutes < 5) {
      setState(() {
        _isSellerApproved = _sellerStatusCache[currentUser.email!]!;
      });
      return;
    }
    ...
    try {
      ...
      // Update cache
      _sellerStatusCache[currentUser.email!] = isApproved;
...
class _BuyerOrdersScreenState extends State<BuyerOrdersScreen>
  ...
  void initState() {
    ...
    // Defer initial load to prevent blocking UI
    Future.microtask(() => _loadOrdersByTab(0));
  }
  ...
  void didUpdateWidget(BuyerOrdersScreen oldWidget) {
    ...
    // Only reload if widget was recreated (not just rebuilt)
  ...
  void refreshOrders() {
    ...
    _lastLoadTime = null; // Reset cache to force refresh
    _loadOrdersByTab(_tabController.index);
  }
  ...
  Future<void> _loadOrdersByTab(int tabIndex) async {
    ...
    // Debounce: Don't reload same tab within 2 seconds
    final now = DateTime.now();
    if (_lastLoadedTab == tabIndex &&
        _lastLoadTime != null &&
        now.difference(_lastLoadTime!).inSeconds < 2) {
      print('⏭️ Skipping redundant load for tab $tabIndex');
      return;
    }

    _lastLoadedTab = tabIndex;
    _lastLoadTime = now;
